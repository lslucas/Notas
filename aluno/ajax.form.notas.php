<?php
	...
	$listAlunos = join(', ', $_POST['alu_id']);
	$msgErr = null;

	if (empty($listAlunos))
		$msgErr .= '<div class="alert alert-error">Nenhum aluno foi selecionado!</div>';

	if (empty($idturma))
		$msgErr .= '<div class="alert alert-error">ID da turma inválido!</div>';

	if (!is_null($msgErr)) {
		echo $msgErr;
		echo "
				<div class='form-actions' align=center>
					<input type='button' value='Voltar' id='form-back-lancamento-notas' class='btn'>
				</div>
			 ";
		 ...
		 *query da turma
		 */
		$whrListAlunos = $listAlunos == 'all' ? null : " AND alu_adm_id IN ({$listAlunos}) ";
		$sqla = "SELECT 
					alu_adm_id,
					alu_id,
		...
		<table width='100%' class="table table-condensed table-striped">
		   ...
		   <tbody>
			<?php
				...
				<tr>
					<?php if ($w==0) { ?>
						<td colspan=2 rowspan=<?=count($listDisciplinas)?>>
							<?php if (!empty($aluno['registro'])) { ?>
							<span class='label'><?=$aluno['registro']?></span>
							<?php } ?>
							<?=$aluno['nome']?>
						</td>
					<?php } ?>
